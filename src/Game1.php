...
{
    protected static $defaultName = 'game:1';
...
//        'eight959tzxkgqjd'
    ...
    protected function solvePartOne(array $input): int {
        ...
        foreach ($input as $line) {
            ...
            foreach ($letters as $char) {
                if (count($lineNumbers) === 0 && is_numeric($char)) {
                    $lineNumbers[] = $char;
            ...
            if (!isset($lineNumbers[1])) {
                $lineNumbers[1] = $lineNumbers[0];
            }

            $testResults[] = $lineNumbers[0] . $lineNumbers[1];
        }

        return array_sum($testResults);
    }
    ...
    protected function solvePartTwo(array $input): int {
        ...
        foreach ($input as $line) {
            $originalLine = $line;
            $order = [];
            foreach ($digitWords as $number => $word) {
                if (str_contains($line, $word)) {
                    $offset = 0;
                    while (strpos($line, $word, $offset) !== false) {
                        $pos = strpos($line, $word, $offset);
                        $offset = $pos + 1;
                        $order[$pos] = $number;
                    }
                }
            ...
            $last = empty($order) ? $first : array_pop($order);

            $testResults[$originalLine . ' - '. implode($fullOrdered)] = $first . $last;
        }
