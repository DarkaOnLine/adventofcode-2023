...
{
    protected static $defaultName = 'game:2';
    ...
    protected function solvePartOne(array $input): int {
        ...
        foreach ($input as $gameNum => $game) {
            $draw = explode(': ', $game)[1];

            $drawSets = explode('; ', $draw);


            foreach ($drawSets as $set) {
                $colors = explode(', ', $set);
                ...
                foreach ($colors as $color) {
                    if (str_contains($color, 'red')) {
                        $colorsCount['red'] += (int)explode(' red', $color)[0];
                    }

                    if (str_contains($color, 'green')) {
                        $colorsCount['green'] += (int)explode(' green', $color)[0];
                    }

                    if (str_contains($color, 'blue')) {
                        $colorsCount['blue'] += (int)explode(' blue', $color)[0];
                    }
                }

                if ($colorsCount['red'] <= 12 && $colorsCount['green'] <= 13 && $colorsCount['blue'] <= 14) {
                    $possibleGames[$gameNum] = $gameNum+1;
                } else {
                    unset($possibleGames[$gameNum]);
                    break;
                }
            }
        }
    ...
    protected function solvePartTwo(array $input): int {
        ...
        foreach ($input as $gameNum => $game) {
            ...
            $setColorsCount = [];
            foreach ($drawSets as $setKey => $set) {
                $colors = explode(', ', $set);


                foreach ($colors as $color) {
                    if (str_contains($color, 'red')) {
                        $setColorsCount[$setKey]['red'] = (int)explode(' red', $color)[0];
                    }

                    if (str_contains($color, 'green')) {
                        $setColorsCount[$setKey]['green'] = (int)explode(' green', $color)[0];
                    }

                    if (str_contains($color, 'blue')) {
                        $setColorsCount[$setKey]['blue'] = (int)explode(' blue', $color)[0];
                    }
                }
            }
            ...
            foreach ($setColorsCount as $colors) {
                foreach ($colors as $color => $num) {
                    $colorMax[$color] = max($num, ($colorMax[$color] ?? 0));
                }
            }

            $power = $colorMax['red'] * $colorMax['green'] * $colorMax['blue'];
