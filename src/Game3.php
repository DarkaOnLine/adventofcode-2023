...
{
    protected static $defaultName = 'game:3';
            ...
            for ($i = 0; $i < count($row); $i++) {
                ...
                if (is_numeric($char)) {
                    while ($char !== '.' && is_numeric($char)) {
                        ...
                        $topLeft = $topRow[$i-1] ?? null;
                        if (isset($topLeft) && $topLeft !== '.' && !is_numeric($topLeft)) {
                            $isPartNumber = true;
                        }

                        $topCenter = $topRow[$i] ?? null;
                        if (isset($topCenter) && $topCenter !== '.' && !is_numeric($topCenter)) {
                            $isPartNumber = true;
                        }

                        $topRight = $topRow[$i+1] ?? null;
                        if (isset($topRight) && $topRight !== '.' && !is_numeric($topRight)) {
                            $isPartNumber = true;
                        }

                        $left = $row[$i-1] ?? null;
                        if (isset($left) && $left !== '.' && !is_numeric($left)) {
                            $isPartNumber = true;
                        }

                        $right = $row[$i+1] ?? null;
                        if (isset($right) && $right !== '.' && !is_numeric($right)) {
                            $isPartNumber = true;
                        }

                        $bottomLeft = $bottomRow[$i-1] ?? null;
                        if (isset($bottomLeft) && $bottomLeft !== '.' && !is_numeric($bottomLeft)) {
                            $isPartNumber = true;
                        }

                        $bottomCenter = $bottomRow[$i] ?? null;
                        if (isset($bottomCenter) && $bottomCenter !== '.' && !is_numeric($bottomCenter)) {
                            $isPartNumber = true;
                        }

                        $bottomRight = $bottomRow[$i+1] ?? null;
                        if (isset($bottomRight) && $bottomRight !== '.' && !is_numeric($bottomRight)) {
                            $isPartNumber = true;
                        }
                ...
                if (!empty($partNumber) && $isPartNumber) {
        ...
        foreach ($matrix as $k => $row) {
            ...
            for ($i = 0; $i < count($row); $i++) {
                ...
                if ($char === '*') {
                    ...
                    if (
                        (isset($topLeft) && is_numeric($topLeft))
                        || (isset($topCenter) && is_numeric($topCenter))
                        || (isset($topRight) && is_numeric($topRight))
                    ) {
                        $foundTop = true;
                        $partNumbersCount++;
                    }
                    ...
                    if (
                        (isset($bottomLeft) && is_numeric($bottomLeft))
                        || (isset($bottomCenter) && is_numeric($bottomCenter))
                        || (isset($bottomRight) && is_numeric($bottomRight))
                    ) {
                        $foundBottom = true;
                        $partNumbersCount++;
                    }
                    ...
                    if ($partNumbersCount === 2) {
                        ...
                        if ($foundTop) {
                            $thisPartNumbers[] = $this->findPartNumber($i, $topRow);
                        }

                        if ($foundBottom) {
                            $thisPartNumbers[] = $this->findPartNumber($i, $bottomRow);
                        }
                        ...
                        if ($foundSame) {
                            ...
                            $j++;
                            $partNumber = '';
                            while (is_numeric($row[$j])) {
                                $partNumber .= $row[$j];
                                $j++;
                            }
                            $thisPartNumbers[] = (int) $partNumber;

                            $j = $i;
                            $j--;
                            $partNumber = '';
                            while (is_numeric($row[$j])) {
                                $partNumber = $row[$j] . $partNumber;
                                $j--;
                            }
                            $thisPartNumbers[] = (int)$partNumber;
                        }

                        $partNumbers[] = array_product($thisPartNumbers);
                    }
     ...
     * @return int
     */
    protected function findPartNumber(int $position, array $row): int
    {
        $partNumber = '';
        $originalPos = $position;

        if (
            is_numeric($row[$position])
            || is_numeric($row[$position-1])
        ) {
            if (is_numeric($row[$position-1])) {
                $position--;
            }

            while (isset($row[$position]) && is_numeric($row[$position])) {
                $position--;
            }
        }
        ...
        while (is_numeric($row[$position])) {
            $partNumber .= $row[$position];
            $position++;
        }
